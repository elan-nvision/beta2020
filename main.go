...
func main() {
	...
	router.Handle("/api/private", negroni.New(
		...
		negroni.Wrap(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			tokenString := r.Header["Id_token"][0]
			...
			if !doesUserExist {
				for {
					...
					if doesNotExist(rcode) {
						sqlStatement := `INSERT INTO elan_ca_details (email, fname, lname, institute, age, rcode, points, no_of_people) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`
						_, err = db.Exec(sqlStatement, email, fname, lname, institute, age, rcode, 0, 0)
						if err != nil {
							panic(err)
						}
						break
			...
			rows.Next()
			err = rows.Scan(&user.Email, &user.Fname, &user.Lname, &user.Institute, &user.Age, &user.Rcode, &user.Points, &user.No_of_people)
			if err != nil {
				fmt.Println("Error in parsing", err)
			}
